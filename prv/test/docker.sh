...
if [[ $1 == "test-cmake" ]]; then
    ...
    cp -R /code/${APPNAME} /code/${APPNAME}-cmake
    exitOnError "test-cmake: Failed to copy to directory : /code/${APPNAME}-cmake"

    cd /code/${APPNAME}-cmake
    exitOnError "test-cmake: Failed to switch to directory : /code/${APPNAME}-cmake"
    ...
    # sudo apt install ./pck/${APPNAME}-0.1.0-Linux.deb
...
if [[ $1 == "test-pip" ]]; then
    ...
    cp -R /code/${APPNAME} /code/${APPNAME}-pip
    exitOnError "test-pip: Failed to copy to directory : /code/${APPNAME}-pip"

    cd /code/${APPNAME}-pip
    exitOnError "test-pip: Failed to switch to directory : /code/${APPNAME}-pip"
